...
proc `keydown=`*(n2d: Nim2d, keydown: proc (nim2d: Nim2d,
    scancode: Scancode)) {.inline.} =
  n2d.keydown = keydown

proc `keyup=`*(n2d: Nim2d, keyup: proc (nim2d: Nim2d, scancode: Scancode)) {.inline.} =
  n2d.keyup = keyup

proc `mousemove=`*(n2d: Nim2d, mousemove: proc (nim2d: Nim2d, x, y, dx,
    dy: int32)) {.inline.} =
  n2d.mousemove = mousemove

proc `mousepressed=`*(n2d: Nim2d, mousepressed: proc(nim2d: Nim2d, x, y: int32,
    button, presses: uint8)) {.inline.} =
  n2d.mousepressed = mousepressed

proc `mousereleased=`*(n2d: Nim2d, mousereleased: proc(nim2d: Nim2d, x,
    y: int32, button, presses: uint8)) {.inline.} =
...
proc `window_size_changed=`*(n2d: Nim2d, window_size_changed: proc (
    nim2d: Nim2d)) {.inline.} =
...
proc `window_focus_gained=`*(n2d: Nim2d, window_focus_gained: proc (
    nim2d: Nim2d)) {.inline.} =
  n2d.window_focus_gained = window_focus_gained

proc `window_focus_lost=`*(n2d: Nim2d, window_focus_lost: proc (
    nim2d: Nim2d)) {.inline.} =
...
# Init
proc newNim2d*(title: string, x, y, width, height: cint, background: tuple[r, g,
    b, a: uint8]): Nim2d =
  ...
  Nim2d(
    ...
    mousepressed: proc(nim2d: Nim2d, x, y: int32, button,
        presses: uint8) = discard,
    mousereleased: proc(nim2d: Nim2d, x, y: int32, button,
        presses: uint8) = discard,
  ...
  while nim2d.running:
    while pollEvent(evt):
      case evt.kind:
        ...
        of MouseMotion:
          nim2d.mousemove(nim2d, evt.motion.x, evt.motion.y, evt.motion.xrel,
              evt.motion.yrel)
        of MouseButtonDown:
          nim2d.mousepressed(nim2d, evt.button.x, evt.button.y,
              evt.button.button, evt.button.clicks)
        of MouseButtonUp:
          nim2d.mousereleased(nim2d, evt.button.x, evt.button.y,
              evt.button.button, evt.button.clicks)
